{function name=departures_style_3
                    ...
                    <tr class="success">
                        ...
                        {foreach $departures as $departure_day}
                            {if $departure_day.count}
                                <th>
                                    <span class="panel-heading">{$departure_day.daytype}</span>
                                </th>
                            {/if}
                            {$d=$d+1}
                        {/foreach}
                    ...
                    {foreach $hours as $hour}
                        <tr>
                            ...
                            {foreach $departures as $departure_day}
                                {if $departure_day.count}
                                <td>
                                    {foreach $departure_day.departures as $hour_d}
                                        {if $hour_d.hour == $hour}
                                            {foreach $hour_d.minutes as $minute}
                                                <a data-daytype-id="{$departure_day.daytype_number}"
                                                   data-trip-no="{$minute.tripnumber}"
                                                   data-line="{$line}"
                                                   data-dir-no="{$dir_no}"
                                                   data-stop-id="{$stop_id}"
                                                   class="trip-show {call check_minute hour=$hour_d.hour minute=$minute.min}">
